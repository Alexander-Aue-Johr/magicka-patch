...
function Test-BehaviorMatrix {
    $patchFailures = @(
        ...
        "level_transition.direct_and_immediate",
        ...
        "patch_settings.available",
        "patch_settings.defaults",
        "patch_settings.round_trip",
        "patch_update.asset_parse",
        "patch_update.version_compare",
        ...
        "game_scene.saved_avatar_initialize",
        "game_scene.saved_npc_template",
        "game_scene.saved_current_manager",
        ...
        "network_spawn_handles.client",
        "network_spawn_handles.server",
...
function Verify-RuntimeEffectiveDiff {
    ...
    if ($auditLines -notcontains "result=PASS" -or
        ...
        @($auditLines | Where-Object { $_ -eq "patch_kind=prefix" }).Count -ne 128 -or
        @($auditLines | Where-Object { $_ -eq "patch_kind=postfix" }).Count -ne 33 -or
        ...
        @($auditLines | Where-Object { $_ -eq "patch_kind=transpiler" }).Count -ne 460) {
...
function Write-ExperimentSummary {
    ...
    $summary.Add("implemented_patches=621")
