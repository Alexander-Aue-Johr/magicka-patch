...
function Verify-RuntimeEffectiveDiff {
    ...
    if ($auditLines -notcontains "result=PASS" -or
        ...
        @($auditLines | Where-Object { $_ -eq "patch_kind=prefix" }).Count -ne 133 -or
        @($auditLines | Where-Object { $_ -eq "patch_kind=postfix" }).Count -ne 34 -or
...
function Write-ExperimentSummary {
    ...
    $summary.Add("implemented_patches=628")
