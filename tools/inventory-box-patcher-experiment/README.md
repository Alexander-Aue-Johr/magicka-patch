...
four hundred forty-four method patches:
...
- Initialized play-state teardown stops active Fairy effects and dialogs,
  deactivates each Fairy, and releases its Character owner, including inactive
...
- The static `ElementalEgg` instance cache and elemental-template lookup are
  ...
  entries.
- The level-owned `Item` pickable queue is released during initialized
...
Avatar pool selection, NonPlayerCharacter, PhysicsEntity, and one
