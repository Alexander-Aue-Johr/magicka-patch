...
{
    internal static class DrinkBloodPlayStatePatch
    {
        private static FieldInfo playStateField;

        internal static readonly RuntimePatchDefinition Definition =
            RuntimePatchDefinition.Transpile(
                "DrinkBlood unused play-state release",
                "org.magickacommunitypatch.drink-blood-play-state",
                FindTarget,
                typeof(DrinkBloodPlayStatePatch).GetMethod("Transpiler"));

        private static MethodInfo FindTarget(Assembly targetAssembly)
        {
            Type drinkBloodType = targetAssembly.GetType(
            ...
            MethodInfo method = drinkBloodType.GetMethod(
                "Execute",
            ...
            if (playStateField == null || playStateField.FieldType != playStateType)
                throw new MissingFieldException(drinkBloodType.FullName, "mPlayState");
            ...
            for (int index = 2; index < result.Count; index++)
            {
                ...
                if (assignment >= 0)
                    throw new InvalidOperationException(
                        "Multiple DrinkBlood play-state assignments matched.");
                assignment = index;
            }
            if (assignment < 0)
                throw new InvalidOperationException(
                    "DrinkBlood play-state assignment was not found.");
