...
{
    internal static class DeflectionAuraPlayStatePatch
    {
        private static FieldInfo playStateField;

        internal static readonly RuntimePatchDefinition Definition =
            RuntimePatchDefinition.Transpile(
                "DeflectionAura unused play-state release",
                "org.magickacommunitypatch.deflection-aura-play-state",
                FindTarget,
                typeof(DeflectionAuraPlayStatePatch).GetMethod("Transpiler"));

        private static MethodInfo FindTarget(Assembly targetAssembly)
        {
            Type auraType = targetAssembly.GetType(
            ...
            MethodInfo method = auraType.GetMethod(
                "Execute",
            ...
            if (playStateField == null || playStateField.FieldType != playStateType)
                throw new MissingFieldException(auraType.FullName, "mPlayState");
            ...
            for (int index = 2; index < result.Count; index++)
            {
                ...
                if (assignment >= 0)
                    throw new InvalidOperationException(
                        "Multiple DeflectionAura play-state assignments matched.");
                assignment = index;
            }
            if (assignment < 0)
                throw new InvalidOperationException(
                    "DeflectionAura play-state assignment was not found.");
