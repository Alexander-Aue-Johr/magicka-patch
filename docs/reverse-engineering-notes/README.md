...
- `network-guard-telemetry.md`: multiplayer guards, telemetry, and the Steam
  authentication pointer workaround.
- `playstate-avatar-cleanup.md`: avoids double-disposing network avatars during
  `PlayState` exit and guards inventory cleanup during `Avatar.Deinitialize()`.
